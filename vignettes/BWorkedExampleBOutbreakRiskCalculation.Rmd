...
dim1=dim(input_data$vacc_data)[2]
dim2=dim(input_data$vacc_data)[3]
model_data <- Model_Run_Many_Reps(FOI_spillover = array(2.5e-8,dim=c(1,1)), 
                                  R0 = array(0.0,dim=c(1,1)), 
                                  vacc_data = array(input_data$vacc_data[1,,],dim = c(1,dim1,dim2)), 
                                  pop_data = array(input_data$pop_data[1,,],dim = c(1,dim1,dim2)), 
                                  years_data = years_data, year0 = input_data$years_labels[1],
                                  vaccine_efficacy = 1.0, time_inc = 1.0, mode_out = 3, mode_start = 1,  
                                  start_SEIRV = NULL, mode_time = 0,  n_reps = n_reps, division = 10, seed = 1)
```

We can calculate the number of reported cases by year for a single repetition using the case_data_calculate() function. The reporting of yellow fever cases is governed by the probability of an infection which causes severe but non-fatal symptoms being reported (p_rep_severe) and the probability of a fatal infection being reported (p_rep_death). It is assumed that infections not leading to severe or fatal symptoms are not reported. The proportion of infections which cause severe symptoms is given by the quantity p_severe_inf, and the proportion of severe infections which lead to death is given by the quantity p_death_severe_inf. These have values 0.12 and 0.39 respectively, based on [Servadio et # al](https://doi.org/10.1186/s12879-021-06535-4).
...
One or more outbreaks are considered to have occurred in a year if the number of reported cases is more than 1, so binary outbreak occurrence is considered to be 1 (true) in years 1, 2, 5, 6 and 10, and 0 (false) in the other years in the above results.
