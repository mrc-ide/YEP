...
# case Annual reported case/death data template - data frame with region and year
...
Generate_Dataset <- function(FOI_values = c(),R0_values = c(),input_data = list(),template = list(sero=NULL,case=NULL),
  ...
  #assert_that(any(is.null(template$sero) == FALSE,is.null(template$case) == FALSE),msg = "Need at least one template")
  ...
  #   assert_that(all(c("region","year") %in% names(template$case)))
  ...
  if(is.null(template$sero) == FALSE){
    if(is.null(xref_sero) | flag_truncated){xref_sero = template_region_xref(template$sero,input_data$region_labels)}
  ...
  if(is.null(template$case) == FALSE){
    if(is.null(xref_case) | flag_truncated){xref_case = template_region_xref(template$case,input_data$region_labels)}
  ...
  if(is.null(region_grouping)){
    region_grouping = get_region_grouping(regions,template$sero,template$case,xref_sero,xref_case)
  }
...
Generate_VIMC_Burden_Dataset <- function(FOI_values = c(),R0_values = c(),input_data = list(),template = NULL,
  ...
  #Cross-reference templates with input regions
  if(is.null(xref) | flag_truncated ){xref = template_region_xref(template,input_data$region_labels)}
  ...
  if(mode_parallel){
    ...
    model_output_all = clusterMap(cl = cluster,fun = Model_Run, FOI_spillover = FOI_subsets, R0 = R0_subsets,
                                  ...
                                  MoreArgs = list(mode_out = 3, year0 = input_data$years_labels[1],
  ...
  for(n_region in 1:n_regions){
    ...
    if(mode_parallel == FALSE){
      ...
      model_output = Model_Run(FOI_spillover = FOI_values[n_region,],R0 = R0_values[n_region,],
                               ...
                               start_SEIRV = start_SEIRV[[n_region]],mode_out = 3,
