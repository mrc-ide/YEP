...
SEIRV_Model <- R6::R6Class(
  ...
  public = list(
    initialize = function(pars, time, n_particles, n_threads = 1L,
      ...
      if (is.null(gpu_config)) {
        private$methods_ <- list(
           ...
           state = dust_cpu_SEIRV_Model_state,
           time = dust_cpu_SEIRV_Model_time,
           reorder = dust_cpu_SEIRV_Model_reorder,
           resample = dust_cpu_SEIRV_Model_resample,
           rng_state = dust_cpu_SEIRV_Model_rng_state,
           set_rng_state = dust_cpu_SEIRV_Model_set_rng_state,
           set_n_threads = dust_cpu_SEIRV_Model_set_n_threads,
           set_data = dust_cpu_SEIRV_Model_set_data,
           compare_data = dust_cpu_SEIRV_Model_compare_data,
           filter = dust_cpu_SEIRV_Model_filter,
           set_stochastic_schedule = dust_cpu_SEIRV_Model_set_stochastic_schedule,
           ode_statistics = dust_cpu_SEIRV_Model_ode_statistics)
      } else {
    ...
    has_openmp = function() {
      dust_cpu_SEIRV_Model_capabilities()[["openmp"]]
    },
    ...
    has_gpu_support = function(fake_gpu = FALSE) {
      if (fake_gpu) {
        ...
        dust_cpu_SEIRV_Model_capabilities()[["gpu"]]
      }
    },

    has_compare = function() {
      dust_cpu_SEIRV_Model_capabilities()[["compare"]]
    },

    real_size = function() {
      dust_cpu_SEIRV_Model_capabilities()[["real_size"]]
    },

    time_type = function() {
      dust_cpu_SEIRV_Model_capabilities()[["time_type"]]
    },

    rng_algorithm = function() {
      dust_cpu_SEIRV_Model_capabilities()[["rng_algorithm"]]
    },
