...
#' @export
#'
mcmc_checks <- function(log_params_ini = c(), n_regions = 1, prior_settings = list(type = "zero"), enviro_data = list(),
                        add_values = list(vaccine_efficacy = 1.0, p_rep_severe = 1.0, p_rep_death = 1.0, m_FOI_Brazil = 1.0),
  ...
  if("p_rep_severe" %in% add_value_names){assert_that("p_rep_death" %in% add_value_names)}
  ...
  for(var_name in add_value_names){
    if(var_name %in% extra_estimated_params){
      assert_that(is.na(add_values[[var_name]]))
