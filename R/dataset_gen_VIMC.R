...
#' @param mode_div Mode of division of regions while running model. Parallel mode will run all years for every region \cr
#'  If mode_div = "parallel", run all regions simultaneously in a single use of the Model_Run function \cr
...
#' @param n_nodes Number of nodes to use in parallelization (if mode_div = cluster)
...
Generate_VIMC_Burden_Dataset <- function(FOI_values = c(),R0_values = c(),input_data = list(),template = NULL,
                                         vaccine_efficacy = 1.0, time_inc = 1.0, mode_start = 1, start_SEIRV = NULL,
  ...
  assert_that(dim(FOI_values)[1] == n_regions && dim(R0_values)[1] == n_regions,
              msg = "1st dimensions of FOI_values and R0_values must match number of regions to be modelled")
