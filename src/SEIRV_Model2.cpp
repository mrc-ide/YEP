...
public:
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[11]] = (flag_year == 1 ? 0 : R_cu[i - 1] + R[i - 1]);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[12]] = (flag_year == 1 ? R_cu[i - 1] + R[i - 1] : 0);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[13]] = (flag_year == 1 ? 0 : SEIR_cu[i - 1] + S[i - 1] + E[i - 1] + I[i - 1] + R[i - 1]);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[14]] = (flag_year == 1 ? SEIR_cu[i - 1] + S[i - 1] + E[i - 1] + I[i - 1] + R[i - 1] : 0);
    }
  }
};
