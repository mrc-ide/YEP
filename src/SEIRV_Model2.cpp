...
public:
  ...
  struct shared_state {
    ...
    struct dim_type {
      ...
      dust2::array::dimensions<1> C_cu;
      dust2::array::dimensions<1> C_annual;
      dust2::array::dimensions<1> SEIR_cu;
      dust2::array::dimensions<1> SEIR_annual;
      dust2::array::dimensions<1> R_cu;
      dust2::array::dimensions<1> R_annual;
  ...
  static shared_state build_shared(cpp11::list parameters) {
    ...
    dim.C_cu.set({static_cast<size_t>(N_age)});
    dim.C_annual.set({static_cast<size_t>(N_age)});
    dim.SEIR_cu.set({static_cast<size_t>(N_age)});
    dim.SEIR_annual.set({static_cast<size_t>(N_age)});
    dim.R_cu.set({static_cast<size_t>(N_age)});
    dim.R_annual.set({static_cast<size_t>(N_age)});
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state[i - 1 + shared.odin.offset.state[9]] = 0;
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    const auto * R_cu = state + shared.odin.offset.state[13];
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[9]] = (flag_year == 1 ? 0 : C_cu[i - 1] + internal.I_new[i - 1]);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[10]] = (flag_year == 1 ? C_cu[i - 1] + internal.I_new[i - 1] : 0);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[11]] = (flag_year == 1 ? 0 : SEIR_cu[i - 1] + S[i - 1] + E[i - 1] + I[i - 1] + R[i - 1]);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[12]] = (flag_year == 1 ? SEIR_cu[i - 1] + S[i - 1] + E[i - 1] + I[i - 1] + R[i - 1] : 0);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[13]] = (flag_year == 1 ? 0 : R_cu[i - 1] + R[i - 1]);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[14]] = (flag_year == 1 ? R_cu[i - 1] + R[i - 1] : 0);
    }
  }
};
