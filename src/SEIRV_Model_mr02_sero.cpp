...
// [[dust2::class(SEIRV_Model_mr02_sero)]]
...
class SEIRV_Model_mr02_sero {
public:
  SEIRV_Model_mr02_sero() = delete;
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.R_cu.mult[1] + shared.odin.offset.state[9]] = 0;
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.R_annual.mult[1] + shared.odin.offset.state[10]] = 0;
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.SEIR_cu.mult[1] + shared.odin.offset.state[11]] = 0;
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.R_cu.mult[1] + shared.odin.offset.state[9]] = (flag_year == 1 ? 0 : R_cu[i - 1 + (j - 1) * shared.dim.R_cu.mult[1]] + R[i - 1 + (j - 1) * shared.dim.R.mult[1]]);
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.R_annual.mult[1] + shared.odin.offset.state[10]] = (flag_year == 1 ? R_cu[i - 1 + (j - 1) * shared.dim.R_cu.mult[1]] + R[i - 1 + (j - 1) * shared.dim.R.mult[1]] : 0);
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.SEIR_cu.mult[1] + shared.odin.offset.state[11]] = (flag_year == 1 ? 0 : SEIR_cu[i - 1 + (j - 1) * shared.dim.SEIR_cu.mult[1]] + S[i - 1 + (j - 1) * shared.dim.S.mult[1]] + E[i - 1 + (j - 1) * shared.dim.E.mult[1]] + I[i - 1 + (j - 1) * shared.dim.I.mult[1]] + R[i - 1 + (j - 1) * shared.dim.R.mult[1]]);
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
...
SEXP dust2_system_SEIRV_Model_mr02_sero_alloc(cpp11::list r_pars, cpp11::sexp r_time, cpp11::list r_time_control, cpp11::sexp r_n_particles, cpp11::sexp r_n_groups, cpp11::sexp r_seed, cpp11::sexp r_deterministic, cpp11::sexp r_n_threads) {
  return dust2::r::dust2_discrete_alloc<SEIRV_Model_mr02_sero>(r_pars, r_time, r_time_control, r_n_particles, r_n_groups, r_seed, r_deterministic, r_n_threads);
}
[[cpp11::register]]
SEXP dust2_system_SEIRV_Model_mr02_sero_run_to_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_run_to_time<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr, r_time);
}
...
SEXP dust2_system_SEIRV_Model_mr02_sero_state(cpp11::sexp ptr, cpp11::sexp r_index_state, cpp11::sexp r_index_particle, cpp11::sexp r_index_group, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_state<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr, r_index_state, r_index_particle, r_index_group, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_system_SEIRV_Model_mr02_sero_time(cpp11::sexp ptr) {
  return dust2::r::dust2_system_time<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRV_Model_mr02_sero_set_state_initial(cpp11::sexp ptr) {
  return dust2::r::dust2_system_set_state_initial<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRV_Model_mr02_sero_set_state(cpp11::sexp ptr, cpp11::list r_state) {
  return dust2::r::dust2_system_set_state<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr, r_state);
}

[[cpp11::register]]
SEXP dust2_system_SEIRV_Model_mr02_sero_reorder(cpp11::sexp ptr, cpp11::integers r_index) {
  return dust2::r::dust2_system_reorder<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr, r_index);
}

[[cpp11::register]]
SEXP dust2_system_SEIRV_Model_mr02_sero_rng_state(cpp11::sexp ptr) {
  return dust2::r::dust2_system_rng_state<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRV_Model_mr02_sero_set_rng_state(cpp11::sexp ptr, cpp11::sexp r_rng_state) {
  return dust2::r::dust2_system_set_rng_state<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr, r_rng_state);
}

[[cpp11::register]]
SEXP dust2_system_SEIRV_Model_mr02_sero_set_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_set_time<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr, r_time);
}

[[cpp11::register]]
SEXP dust2_system_SEIRV_Model_mr02_sero_update_pars(cpp11::sexp ptr, cpp11::list pars) {
  return dust2::r::dust2_system_update_pars<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr, pars);
}
...
SEXP dust2_system_SEIRV_Model_mr02_sero_simulate(cpp11::sexp ptr, cpp11::sexp r_times, cpp11::sexp r_index_state, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_simulate<dust2::dust_discrete<SEIRV_Model_mr02_sero>>(ptr, r_times, r_index_state, preserve_particle_dimension, preserve_group_dimension);
}
