...
SEXP SEIRV_Model_contents(SEXP internal_p) {
  ...
  SEXP contents = PROTECT(allocVector(VECSXP, 85));
  ...
  SET_VECTOR_ELT(contents, 57, ScalarReal(internal->initial_time));
  ...
  SET_VECTOR_ELT(contents, 58, initial_V);
  SET_VECTOR_ELT(contents, 59, ScalarReal(internal->initial_year));
  ...
  SET_VECTOR_ELT(contents, 60, inv_P);
  ...
  SET_VECTOR_ELT(contents, 61, inv_P_nV);
  SET_VECTOR_ELT(contents, 62, ScalarInteger(internal->N_age));
  SET_VECTOR_ELT(contents, 63, ScalarInteger(internal->n_years));
  SET_VECTOR_ELT(contents, 64, ScalarInteger(internal->offset_variable_C));
  SET_VECTOR_ELT(contents, 65, ScalarInteger(internal->offset_variable_E));
  SET_VECTOR_ELT(contents, 66, ScalarInteger(internal->offset_variable_I));
  SET_VECTOR_ELT(contents, 67, ScalarInteger(internal->offset_variable_R));
  SET_VECTOR_ELT(contents, 68, ScalarInteger(internal->offset_variable_V));
  SEXP P = PROTECT(allocVector(REALSXP, internal->dim_P));
  memcpy(REAL(P), internal->P, internal->dim_P * sizeof(double));
  SET_VECTOR_ELT(contents, 69, P);
  SEXP P_nV = PROTECT(allocVector(REALSXP, internal->dim_P_nV));
  memcpy(REAL(P_nV), internal->P_nV, internal->dim_P_nV * sizeof(double));
  SET_VECTOR_ELT(contents, 70, P_nV);
  SET_VECTOR_ELT(contents, 71, ScalarReal(internal->Pmin));
  ...
  SET_VECTOR_ELT(contents, 80, vacc_rate);
  ...
  SET_VECTOR_ELT(contents, 81, vacc_rate_annual);
  SET_VECTOR_ELT(contents, 82, ScalarReal(internal->vaccine_efficacy));
  SET_VECTOR_ELT(contents, 83, ScalarReal(internal->year0));
  SET_VECTOR_ELT(contents, 84, ScalarLogical(internal->SEIRV_Model_use_dde));
  SEXP nms = PROTECT(allocVector(STRSXP, 85));
  ...
  SET_STRING_ELT(nms, 57, mkChar("initial_time"));
  SET_STRING_ELT(nms, 58, mkChar("initial_V"));
  SET_STRING_ELT(nms, 59, mkChar("initial_year"));
  SET_STRING_ELT(nms, 60, mkChar("inv_P"));
  SET_STRING_ELT(nms, 61, mkChar("inv_P_nV"));
  SET_STRING_ELT(nms, 62, mkChar("N_age"));
  SET_STRING_ELT(nms, 63, mkChar("n_years"));
  SET_STRING_ELT(nms, 64, mkChar("offset_variable_C"));
  SET_STRING_ELT(nms, 65, mkChar("offset_variable_E"));
  SET_STRING_ELT(nms, 66, mkChar("offset_variable_I"));
  SET_STRING_ELT(nms, 67, mkChar("offset_variable_R"));
  SET_STRING_ELT(nms, 68, mkChar("offset_variable_V"));
  SET_STRING_ELT(nms, 69, mkChar("P"));
  SET_STRING_ELT(nms, 70, mkChar("P_nV"));
  SET_STRING_ELT(nms, 71, mkChar("Pmin"));
  SET_STRING_ELT(nms, 72, mkChar("R_new"));
  SET_STRING_ELT(nms, 73, mkChar("R0"));
  SET_STRING_ELT(nms, 74, mkChar("Rec0"));
  SET_STRING_ELT(nms, 75, mkChar("Sus0"));
  SET_STRING_ELT(nms, 76, mkChar("t_incubation"));
  SET_STRING_ELT(nms, 77, mkChar("t_infectious"));
  SET_STRING_ELT(nms, 78, mkChar("t_latent"));
  SET_STRING_ELT(nms, 79, mkChar("Vac0"));
  SET_STRING_ELT(nms, 80, mkChar("vacc_rate"));
  SET_STRING_ELT(nms, 81, mkChar("vacc_rate_annual"));
  SET_STRING_ELT(nms, 82, mkChar("vaccine_efficacy"));
  SET_STRING_ELT(nms, 83, mkChar("year0"));
  SET_STRING_ELT(nms, 84, mkChar("SEIRV_Model_use_dde"));
...
SEXP SEIRV_Model_set_user(SEXP internal_p, SEXP user) {
  ...
  internal->offset_variable_C = internal->dim_E + internal->dim_I + internal->dim_R + internal->dim_S + internal->dim_V + 4;
  internal->offset_variable_E = internal->dim_S + 4;
  internal->offset_variable_I = internal->dim_E + internal->dim_S + 4;
  internal->offset_variable_R = internal->dim_E + internal->dim_I + internal->dim_S + 4;
  internal->offset_variable_V = internal->dim_E + internal->dim_I + internal->dim_R + internal->dim_S + 4;
...
SEXP SEIRV_Model_set_initial(SEXP internal_p, SEXP step_ptr, SEXP state_ptr) {
  ...
  if (state_ptr != R_NilValue) {
    ...
    internal->initial_time = state[0];
    internal->initial_day = state[1];
    internal->initial_year = state[2];
    internal->initial_FOI_total = state[3];
    memcpy(internal->initial_S, state + 4, internal->dim_S * sizeof(double));
...
SEXP SEIRV_Model_metadata(SEXP internal_p) {
  ...
  SET_VECTOR_ELT(variable_length, 4, ScalarInteger(internal->dim_S));
  SET_VECTOR_ELT(variable_length, 5, ScalarInteger(internal->dim_E));
  SET_VECTOR_ELT(variable_length, 6, ScalarInteger(internal->dim_I));
  SET_VECTOR_ELT(variable_length, 7, ScalarInteger(internal->dim_R));
  SET_VECTOR_ELT(variable_length, 8, ScalarInteger(internal->dim_V));
  SET_VECTOR_ELT(variable_length, 9, ScalarInteger(internal->dim_C));
  SET_STRING_ELT(variable_names, 0, mkChar("time"));
  SET_STRING_ELT(variable_names, 1, mkChar("day"));
  SET_STRING_ELT(variable_names, 2, mkChar("year"));
  SET_STRING_ELT(variable_names, 3, mkChar("FOI_total"));
  SET_STRING_ELT(variable_names, 4, mkChar("S"));
  SET_STRING_ELT(variable_names, 5, mkChar("E"));
  SET_STRING_ELT(variable_names, 6, mkChar("I"));
  SET_STRING_ELT(variable_names, 7, mkChar("R"));
  SET_STRING_ELT(variable_names, 8, mkChar("V"));
  SET_STRING_ELT(variable_names, 9, mkChar("C"));
...
SEXP SEIRV_Model_initial_conditions(SEXP internal_p, SEXP step_ptr) {
  ...
  SEIRV_Model_internal *internal = SEIRV_Model_get_internal(internal_p, 1);
  SEXP r_state = PROTECT(allocVector(REALSXP, internal->dim_C + internal->dim_E + internal->dim_I + internal->dim_R + internal->dim_S + internal->dim_V + 4));
  double * state = REAL(r_state);
  state[0] = internal->initial_time;
  state[1] = internal->initial_day;
  state[2] = internal->initial_year;
  state[3] = internal->initial_FOI_total;
...
void SEIRV_Model_rhs(SEIRV_Model_internal* internal, size_t step, double * state, double * state_next, double * output) {
  ...
  {
     int i = 1;
     state_next[4 + i - 1] = fmax(internal->Pmin, S[0] - internal->E_new[0] - internal->vacc_rate[0] * S[0] * internal->inv_P_nV[0] + internal->dP1[0] - (internal->dP2[0] * S[0] * internal->inv_P[0]));
  }
  for (int i = 2; i <= internal->N_age; ++i) {
    state_next[4 + i - 1] = fmax(internal->Pmin, S[i - 1] - internal->E_new[i - 1] - internal->vacc_rate[i - 1] * S[i - 1] * internal->inv_P_nV[i - 1] + (internal->dP1[i - 1] * S[i - 1 - 1] * internal->inv_P[i - 1 - 1]) - (internal->dP2[i - 1] * S[i - 1] * internal->inv_P[i - 1]));
  }
