...
public:
  ...
  static real_type compare_data(real_type time, const real_type* state, const data_type& data, const shared_state& shared, internal_state& internal, rng_state_type& rng_state) {
    ...
    for (size_t i = 1; i <= shared.dim.obs_case_values.size; ++i) {
      odin_ll += unless_nan(monty::density::negative_binomial_mu(data.obs_case_values[i - 1], std::numeric_limits<real_type>::infinity(), output_case[i - 1] + static_cast<real_type>(0.001), true));
    }
    for (size_t i = 1; i <= shared.dim.obs_death_values.size; ++i) {
      odin_ll += unless_nan(monty::density::negative_binomial_mu(data.obs_death_values[i - 1], std::numeric_limits<real_type>::infinity(), output_death[i - 1] + static_cast<real_type>(0.001), true));
    }
    return odin_ll;
  }
};
