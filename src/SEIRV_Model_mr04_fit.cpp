...
public:
  ...
  static shared_state build_shared(cpp11::list parameters) {
    ...
    dim.infs_cu.set({static_cast<size_t>(n_regions), static_cast<size_t>(N_age)});
    dim.R_an.set({static_cast<size_t>(n_sero_pts), static_cast<size_t>(n_regions)});
    dim.SEIR_an.set({static_cast<size_t>(n_sero_pts), static_cast<size_t>(n_regions)});
    dim.V_an.set({static_cast<size_t>(n_sero_pts), static_cast<size_t>(n_regions)});
    dim.infs_an.set({static_cast<size_t>(n_case_pts), static_cast<size_t>(n_regions)});
    ...
    dim.region_index_sero.set({static_cast<size_t>(n_sero_pts), static_cast<size_t>(n_regions)});
    dim.region_index_case.set({static_cast<size_t>(n_case_pts), static_cast<size_t>(n_regions)});
    dim.sero_regions.set({static_cast<size_t>(n_regions)});
    dim.case_regions.set({static_cast<size_t>(n_regions)});
    ...
    return shared_state{odin, dim, time_inc, n_regions, t_incubation, t_latent, t_infectious, N_age, vaccine_efficacy, n_sero_pts, n_case_pts, p_severe_inf, p_death_severe_inf, p_rep_severe, p_rep_death, overdisp, year0, n_years, n_t_pts, Pmin, FOI_max, rate1, rate2, FOI_spillover, R0, vacc_rate_daily, region_index_sero, region_index_case, sero_regions, case_regions, sero_vc_factor, sia_min, sia_max, S_0, E_0, I_0, R_0, V_0, dP1_all, dP2_all};
  }
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_sero_pts); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.n_regions); ++j) {
